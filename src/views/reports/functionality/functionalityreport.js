...
import DraftFunctionalReports from './draftedfunctionality'
import NonFunctionalReports from './nonfunctional'
...
currentProjects.map((e,i)=>{
        ...
        <div>
 <div style={{margin:3}}>
      <button className='btn btn-defult btn-primary' onClick={()=>{this.setState({draft:!this.state.draft})}} >
          Functionality
</button></div>

       { this.state.draft &&    <DraftFunctionalReports/>}
      {!this.state.draft && <NonFunctionalReports/>}
      {//<div>
}
 {/*  <div className='row'>
          <div>
